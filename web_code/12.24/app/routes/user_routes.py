...
def product_public(product_id):
    ...
    if product and product.production_status == 'Approved' and product.display_status == 'on display':
        return render_template('user/public_search/product_public.html', product=product)
...
def request_production_round(product_id):
    """Request a new production round for a product."""
    if 'user_id' not in session or session['role'] != 'artist':
    ...
    try:
        ...
        # Ensure no existing active production round
        existing_round = ProductionRound.query.filter_by(product_id=product.id, status='active').first()
        if existing_round:
            flash("A production round already exists for this product.")
            return redirect(url_for('user.artist_dashboard'))

        # Assign a max_waiting_time default if not provided
        max_waiting_time_days = 30  # Example: Default to 30 days from now
        max_waiting_time = datetime.utcnow() + timedelta(days=max_waiting_time_days)
        ...
        flash("Production round requested successfully.")
        return render_template('user/artist/production_round_request_confirmation.html', product=product)
...
@user_interface.route('/product_dialog/<uuid:round_id>', methods=['GET', 'POST'])
def artist_product_dialog(round_id):
    if 'user_id' not in session or session['role'] != 'artist':
        flash("You need to be logged in as an Artist to access this dialog.")
        return redirect(url_for('user.login'))

    production_round = ProductionRound.query.get(round_id)
    if not production_round or production_round.artist_id != uuid.UUID(session['user_id']):
    ...
    if request.method == 'POST':
        ...
        dialog_link = url_for('user.artist_product_dialog', round_id=production_round.id, _external=True)
