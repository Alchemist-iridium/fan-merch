...
class ProductionRound(db.Model):
    ...
    max_waiting_time = db.Column(db.DateTime, nullable=False)  # Changed to DateTime

    production_stage = db.Column(
        db.String(50),
        nullable=False,
        default="minimum production"
    )
    # hmm, actually is the sub-status for the "active" status?
    # but maybe separate it would be good for management? I'm not sure about it...
    ...
    status = db.Column(
        ...
        nullable=False,
        default="requested"
    )  # Possible values: "requested", "active", "finished", "abandoned"

    # customer take orders, which contains multiple items.
    # items are individual product
    # stage is related to the gift given once the quantity reaches...

    items = db.relationship("Item", back_populates="production_round", lazy=True, cascade="all, delete-orphan")
    stages = db.relationship("ProductionStageGoal", back_populates="production_round", lazy=True, cascade="all, delete-orphan")
    ...
    # the items are going to be the individual unit
    ...
    def update_status(self):
        """Check and update the status based on max_waiting_time and order quantity."""
        if self.status == 'active' and datetime.now(timezone.utc) >= self.max_waiting_time:
            if self.current_order_quantity < self.min_production_size:
...
class ProductionStageGoal(db.Model):
    __tablename__ = 'production_stage_goals'
    ...
    gift = db.Column(db.String(255), nullable=False)  # Description of the gift

    # Relationship back to the production round
    production_round = db.relationship("ProductionRound", back_populates="stages")
