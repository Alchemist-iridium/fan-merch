...
class ItemOrderItem(db.Model):
    ...
    item_status = db.Column(db.String(50), default="item")  # Tracks the current stage:"item", "refunded","transferred","delivery process""delivered"
...
class WarehouseStorage(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid6)
    location_name = db.Column(db.String(255), nullable=False, unique=True)  # Unique location identifier (e.g., "A1", "B2").
    size = db.Column(db.Integer, nullable=False)  # Storage size, 1,2,3. 1 is the smallest and 3 is the largest.
    ...
    def __repr__(self):
        return f"<WarehouseStorage(location_name={self.location_name}, is_available={self.is_available})>"
...
class WarehouseRecord(db.Model):
    ...
    warehouse_storage_id = db.Column(UUID(as_uuid=True), db.ForeignKey('warehouse_storages.id'), nullable=False)  # New relationship
    ...
    storage_location = db.relationship("WarehouseStorage", back_populates="warehouse_records")  # New relationship
...
class DeliveryOrder(db.Model):
    ...
    status = db.Column(db.String(50), nullable=False, default="created")    #created, in_process, delivered
    timestamp = db.Column(db.DateTime, default=datetime.now(timezone.utc))

    delivery_item_quantities = db.Column(JSON, nullable=False, default=list)  # Example: [{"production_round_id": "uuid", "quantity": 10}]
    ...
    def calculate_delivery_cost(region, items):
        """
        Calculate the delivery cost for a delivery order.
        
        :param region: The selected delivery region (string).
        :param items: A list of ItemOrderItem objects.
        :return: The total delivery cost (float).
        """
        # Simulate region multipliers
        REGION_MULTIPLIERS = {
            "region_1": 1.5,  # Example: Region 1 multiplier is 1.5
            "region_2": 2.0,
            "region_3": 2.5,
            "region_4": 3.0
        }
        ...
        # Get the multiplier for the selected region
        region_multiplier = REGION_MULTIPLIERS[region]
        
        # Sum the delivery points of the selected items
        total_delivery_points = sum(item.production_round.delivery_point for item in items)
        
        # Calculate the total cost
        delivery_cost = region_multiplier * total_delivery_points
        
        return round(delivery_cost, 2)  # Return the rounded cost
