...
def create_warehouse_storage():
    ...
    if request.method == 'POST':
        ...
        if not location_name or not size:
            flash("Location name and size are required.", "danger")
...
def search_warehouse_storage():
    """Search for storage locations by location name and size."""
    if 'user_id' not in session or session['role'] != 'warehouse_admin':
        flash("You must log in as a Warehouse Admin to access this page.")
        return redirect(url_for('admin_interface.admin_login'))
...
@admin_interface.route('/warehouse_admin/storage/delete/<uuid6:storage_id>', methods=['POST'])
def delete_warehouse_storage(storage_id):
    """Delete a storage location if it is available."""
    if 'user_id' not in session or session['role'] != 'warehouse_admin':
        flash("You must log in as a Warehouse Admin to access this page.")
        return redirect(url_for('admin_interface.admin_login'))

    storage = WarehouseStorage.query.get(storage_id)
    if not storage:
...
@admin_interface.route('/warehouse_admin/storage/view_record/<uuid6:storage_id>', methods=['GET'])
def view_warehouse_record(storage_id):
    ...
    warehouse_record = WarehouseRecord.query.filter_by(warehouse_storage_id=storage_id).first()
...
def manage_warehouse_record(production_round_id):
    ...
    # Preload available storage by size
    available_storage = {
        1: WarehouseStorage.query.filter_by(size=1, is_available=True).all(),
        2: WarehouseStorage.query.filter_by(size=2, is_available=True).all(),
        3: WarehouseStorage.query.filter_by(size=3, is_available=True).all(),
    }
    ...
    if request.method == 'POST':
        try:
            ...
            # Select the first available storage location for the given size
            storage_location = WarehouseStorage.query.filter_by(size=size, is_available=True).first()
            if not storage_location:
                flash("No available storage location found for the selected size.", "danger")
            ...
            warehouse_record = WarehouseRecord(
                ...
                warehouse_storage_id=storage_location.id,
                quantity=quantity,
...
def delete_warehouse_record(record_id):
    ...
    try:
        ...
        storage_location = WarehouseStorage.query.get(warehouse_record.warehouse_storage_id)
        if storage_location:
