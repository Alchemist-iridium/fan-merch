    ...
    try:
        ...
        category_filters = {
            "in_production": items_query.join(ProductionRound).filter(
                ...
                ProductionRound.stage.in_(["waiting", "sample", "production", "examination"])
            ),
            ...
            "delivered": items_query.join(ProductionRound).filter(
                ProductionRound.is_active.is_(False),
                ProductionRound.stage == "stocking",
                ItemOrderItem.item_status == "delivered"
            ),
        ...
        # Render template with items and category
        return render_template(
            'user/customer/item_management.html',
            items=items,
            category=category,
        )
...
# delivery order opereation for customer

@user_interface.route('/put_delivery_order', methods=['POST'])
def put_delivery_order():
    """
    Save selected items for delivery order, aggregate them, and redirect to place_delivery_order.
    """
    if 'user_id' not in session or session['role'] != 'customer':
    ...
    try:
        ...
        selected_item_ids = request.form.getlist('selected_items')  # List of item IDs
        if not selected_item_ids:
            flash("No items selected for delivery order.")
            return redirect(url_for('user_interface.item_management', category='waiting_for_delivery'))

        # Query the selected items
        selected_items = ItemOrderItem.query.filter(ItemOrderItem.id.in_(selected_item_ids)).all()
        if not selected_items:
            flash("Selected items not found.")
            return redirect(url_for('user_interface.item_management', category='waiting_for_delivery'))

        # Group items by production_round_id
        production_round_quantities = {}
        for item in selected_items:
            production_round_id = str(item.production_round_id)  # Convert UUID to string for JSON compatibility
            production_round_quantities[production_round_id] = production_round_quantities.get(production_round_id, 0) + 1

        # Save aggregated data in session
        session['production_round_quantities'] = production_round_quantities
        session['selected_item_ids'] = selected_item_ids

        flash("Items successfully added to delivery order.")
        return redirect(url_for('user_interface.place_delivery_order'))

    except Exception as e:
        logging.exception("Error in put_delivery_order: %s", e)
        flash("An error occurred while processing your delivery order.")
        return redirect(url_for('user_interface.item_management', category='waiting_for_delivery'))
...
@user_interface.route('/place_delivery_order', methods=['GET', 'POST'])
def place_delivery_order():
    """
    Display aggregated items, calculate delivery cost, and handle order creation with payment method selection.
    """
    if 'user_id' not in session or session['role'] != 'customer':
        flash("You need to log in to proceed.")
        return redirect(url_for('user_interface.login'))
    ...
    try:
        # Retrieve aggregated data from session
        production_round_quantities = session.get('production_round_quantities', {})
        selected_item_ids = session.get('selected_item_ids', [])
        if not production_round_quantities or not selected_item_ids:
            flash("No items selected for delivery order.")
            return redirect(url_for('user_interface.item_management', category='waiting_for_delivery'))

        # Query the selected items for display
        selected_items = ItemOrderItem.query.filter(ItemOrderItem.id.in_(selected_item_ids)).all()

        # Calculate delivery cost
        region = session.get('region', 'region_1')  # Default region if not set
        delivery_cost = calculate_delivery_cost(region, production_round_quantities)
        ...
        if request.method == 'POST':
            ...
            if not payment_method or not shipping_address:
                flash("Please select a payment method and enter a shipping address.")
                return render_template(
                    'user/customer/place_delivery_order.html',
            ...
            delivery_order = DeliveryOrder(
                ...
                delivery_item_quantities=[
                    {"production_round_id": k, "quantity": v} for k, v in production_round_quantities.items()
                ],
            ...
            session['delivery_order_id'] = str(delivery_order.id)

            # Redirect to the appropriate payment route
            if payment_method == 'wallet':
                return redirect(url_for('user_interface.delivery_wallet_payment'))
            elif payment_method == 'credit_card':
                return redirect(url_for('user_interface.delivery_credit_card_payment'))
            else:
        ...
        logging.exception("Error in place_delivery_order: %s", e)
        flash("An error occurred while processing your delivery order.")
        return redirect(url_for('user_interface.item_management', category='waiting_for_delivery'))
...
@user_interface.route('/delivery_wallet_payment', methods=['GET', 'POST'])
def delivery_wallet_payment():
    """
    Handle wallet payment for the delivery order.
    """
    if 'user_id' not in session or session['role'] != 'customer':
        flash("You need to log in to proceed.")
        return redirect(url_for('user_interface.login'))
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if customer.wallet_balance < delivery_cost:
                flash("Insufficient wallet balance.")
                return redirect(url_for('user_interface.delivery_wallet_payment'))

            # Deduct balance and create delivery order
            customer.wallet_balance -= delivery_cost
            delivery_order = DeliveryOrder(
            ...
            flash("Payment successful and delivery order placed.")
            return redirect(url_for('user_interface.customer_dashboard'))

        return render_template(
            'user/customer/delivery_wallet_payment.html',
            delivery_cost=delivery_cost
        )
    except Exception as e:
        logging.exception("Error in wallet_payment: %s", e)
...
@user_interface.route('/view_delivery_order_detail/<uuid6:order_id>', methods=['GET'])
...
def item_order_wallet_payment():
    try:
        ...
        if request.method == 'POST':
            ...
            # Deduct wallet balance and complete payment
            try:
                ...
                logging.exception(f"Error during payment processing: {e}")
                db.session.rollback()
