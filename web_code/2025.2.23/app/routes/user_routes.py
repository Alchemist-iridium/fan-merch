...
def waiting_for_delivery():
    if 'user_id' not in session or session['role'] != 'customer':
        flash("You must log in as a Customer to access this page.", "error")
        return redirect(url_for('user_interface.login'))
...
def preview_delivery_order():
    ...
    warehouse_id = WarehouseRegionMapping.query.filter_by(region_id=region_id).first().warehouse_id
    ...
    try:
        delivery_order = DeliveryOrder(
            ...
            status="created",
            delivery_item=[{"item_id": str(item.id), "production_round_id": str(item.production_round_id)} for item in items],
...
def view_refund_orders():
    ...
    try:
        if 'user_id' not in session or session.get('role') != 'customer':
            logging.warning("Unauthorized access attempt to /customer/view_refund_orders.")
            flash("You need to be logged in as a customer to view your refund orders.")
            return redirect(url_for('user_interface.login'))

        customer_id = uuid.UUID(session['user_id'])  # Convert string to UUID object
        logging.debug("Fetching refund orders for Customer ID: %s", customer_id)

        refund_orders = RefundItemOrder.query.filter_by(customer_id=customer_id).order_by(RefundItemOrder.created_at.desc()).all()

        if not refund_orders:
            logging.info("No refund orders found for Customer ID: %s", customer_id)
        else:
            logging.info("Retrieved %d refund orders for Customer ID: %s", len(refund_orders), customer_id)
        ...
        logging.exception("Unexpected error in /customer/view_refund_orders route: %s", e)
...
@user_interface.route('/customer/view_refund_order_detail/<uuid:refund_id>', methods=['GET'])
def view_refund_order_detail(refund_id):
    """
    Displays details of a specific refund order for the logged-in customer.
    """
    if 'user_id' not in session or session.get('role') != 'customer':
        logging.warning("Unauthorized access attempt to /customer/view_refund_order_detail.")
        flash("You need to be logged in as a customer to view refund details.")
        return redirect(url_for('user_interface.login'))

    customer_id = uuid.UUID(session['user_id'])  # Convert string to UUID object
    logging.debug("Fetching refund order detail for Customer ID: %s, Refund ID: %s", customer_id, refund_id)

    refund_order = RefundItemOrder.query.filter_by(id=refund_id, customer_id=customer_id).first()
    if not refund_order:
        logging.info("Refund order %s not found or not owned by Customer ID: %s", refund_id, customer_id)
...
def confirm_checkout():
    ...
    if 'user_id' not in session or session['role'] != 'customer':
        flash("Please log in as a customer.", "error")
        logging.error(f"Session validation failed: user_id={session.get('user_id')}, role={session.get('role')}")
        return redirect(url_for('user_interface.login'))
    ...
    transaction_fee = 1.20 if payment_method == 'wallet' else round(subtotal * 0.03, 2)
    ...
    try:
        ...
        if not customer:
            ...
            logging.error(f"No customer found for ID: {customer_id}")
            return redirect(url_for('user_interface.home'))
    except ValueError as e:
        logging.exception(f"Invalid customer_id in session: {session['user_id']}")
...
def pay_unpaid_item_order(order_id):
    """Handle payment continuation for unpaid item orders."""
    try:
        # Verify user session and role
        if 'user_id' not in session or session['role'] != 'customer':
        ...
        # Handle POST: Redirect based on payment method
        if request.method == 'POST':
            payment_method = request.form.get('payment_method')
            logging.debug(f"Selected payment method: {payment_method}")
            if payment_method == 'wallet':
                return redirect(url_for('user_interface.item_order_process_wallet_payment', order_id=item_order.id))
            elif payment_method == 'credit_card':
                flash("Credit card payment is not yet supported.", "error")
                return redirect(url_for('user_interface.pay_unpaid_item_order', order_id=item_order.id))
            elif payment_method == 'paypal':
                flash("PayPal payment is not yet supported.", "error")
                return redirect(url_for('user_interface.pay_unpaid_item_order', order_id=item_order.id))
            else:
        ...
        # GET request: Render payment selection page
        return render_template('user/customer/pay_unpaid_item_order.html', item_order=item_order)

    except Exception as e:
        logging.exception(f"Unexpected error in /pay_unpaid_item_order route: {e}")
        flash(f"An error occurred: {e}", "error")
        return redirect(url_for('user_interface.view_item_orders'))
...
@user_interface.route('/auto_refund_item/<uuid:item_id>', methods=['GET'])
def auto_refund_item(item_id):
    """
    Displays the refund details for a specific item.
    """
    if 'user_id' not in session or session['role'] != 'customer':
    ...
    try:
        ...
        if item.production_round.stage == "waiting":
            base_refund_amount = item.unit_price  # Full Refund
            refund_type = "Full Refund"
        elif item.production_round.stage == "sample":
            base_refund_amount = item.production_round.partial_refund  # Partial Refund
            refund_type = "Partial Refund"
        else:
            flash("This item is not eligible for a refund.")
            return redirect(url_for('user_interface.customer_dashboard'))

        # Calculate total refund with tax using region_id
        region = Region.query.get(item.region_id)
        if not region:
            logging.warning(f"Region ID {item.region_id} not found for item {item_id}")
            tax_rate = 0.0  # Fallback if region is missing
            total_refund_amount = base_refund_amount
            region_name = "Unknown Region"
        else:
            tax_rate = region.tax_rate
            tax_amount = base_refund_amount * tax_rate
            total_refund_amount = base_refund_amount + tax_amount  # Base + tax refund
            region_name = region.name

        # Fetch related product and artwork
        ...
        if not product:
            flash("Product details not found.")
            return redirect(url_for('user_interface.customer_dashboard'))

        if not artwork:
            flash("Artwork details not found.")
            return redirect(url_for('user_interface.customer_dashboard'))
...
def process_auto_refund_wallet():
    ...
    try:
        ...
        # Get region-based tax
        region = item.region  # Assuming `region` is an attribute of ItemOrderItem
        total_refund_amount = get_tax_for_region(region, base_refund_amount)
        ...
        flash(f"Refund processed successfully. ${total_refund_amount} has been added to your wallet.", "success")
        return redirect(url_for('user_interface.customer_dashboard'))
...
# region based tax amount calculator, transaction fee not included
def get_tax_for_region(region, subtotal):
    """ Calls the tax calculation API and returns (total_amount). """
    try:
        response = requests.post(
            url=url_for('user_interface.calculate_tax_for_refund', _external=True),
            json={"region": region, "subtotal": subtotal},
            headers={"Content-Type": "application/json"}
        )
        if response.status_code == 200:
            data = response.json()
            return data.get("total_amount", subtotal)
        else:
            logging.error(f"Tax API error: {response.text}")
            return 0, subtotal  # Return default values on error
    except Exception as e:
        logging.exception("Error fetching tax data: %s", e)
        return 0, subtotal  # Fail-safe return
