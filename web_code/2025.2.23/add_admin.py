# add_admin.py
from main import db
from app.models import ArtworkApprovalAdmin, OrderManagementAdmin, ProductApprovalAdmin, WarehouseAdmin, DeliveryAdmin, CustomerServiceAdmin,InfoAdmin
...
def add_admin(name, email, password, role):
    with app.app_context():
        ...
        if ArtworkApprovalAdmin.query.filter_by(email=email).first() or \
           ...
           InfoAdmin.query.filter_by(email=email).first() or \
           CustomerServiceAdmin.query.filter_by(email=email).first():
        ...
        if role == 'artwork_approval_admin':
            ...
            print(f"Invalid role '{role}' specified. Please choose from 'artwork_approval_admin', 'order_management_admin', 'product_approval_admin', 'info_admin', or 'customer_service_admin'.")
            return
...
if __name__ == '__main__':
    ...
    role = input("Enter the role for the new admin (artwork_approval_admin, order_management_admin, product_approval_admin, warehouse_admin, delivery_admin, info_admin, customer_service_admin): ").strip().lower()
