...
def add_to_cart(round_id):
    try:
        ...
        if not production_round.is_published:
            ...
            return redirect(url_for('user_interface.home'))
        print(f"[DEBUG] Production Round ID: {round_id}, Product Name: {production_round.product.name}")

        quantity = int(request.form['quantity'])
        if quantity < 1:
            raise ValueError("Quantity must be at least 1.")
        print(f"[DEBUG] Quantity to add: {quantity}")

        # Retrieve the unit price for display
        unit_price_for_display = production_round.price
        print(f"[DEBUG] Unit Price for Display: {unit_price_for_display}")

        # Initialize the cart if it doesn't exist
        if not customer.cart:
            print("[DEBUG] Cart does not exist. Creating a new cart.")
            customer.cart = Cart(customer_id=customer.id, items={})  # Ensure items is initialized
            db.session.add(customer.cart)

        product = production_round.product
        artwork = product.artwork

        customer.cart.add_item(
            production_round_id=round_id,
            quantity=quantity,
            unit_price=production_round.price,
            product_name=product.name,
            product_image_path=artwork.image_path
        )

        db.session.commit()
        flash(f"Added {quantity} item(s) from '{product.name}' to your cart.", "success")

        print(f"[DEBUG] Cart after adding item: {customer.cart.items}")
...
def view_cart():
    try:
        ...
        print(f"[DEBUG] Customer cart before rendering: {customer.cart.items}")
        cart_items = customer.cart.get_cart_items()
        total_amount = customer.cart.calculate_total()
        print(f"[DEBUG] Total Amount: {total_amount}, Cart Items: {cart_items}")

        return render_template('user/customer/view_cart.html', cart_items=cart_items, total_amount=total_amount)
    except Exception as e:
        print(f"[ERROR] Exception occurred in view_cart: {e}")
...
def update_cart_quantity():
    try:
        ...
        print(f"[DEBUG] Updating cart quantity. Round ID: {production_round_id}, New Quantity: {new_quantity}")
        if new_quantity <= 0:
            customer.cart.remove_item(production_round_id)
        else:
            customer.cart.update_item_quantity(production_round_id, new_quantity)

        db.session.commit()
        print(f"[DEBUG] Cart after updating quantity: {customer.cart.items}")
        return jsonify({"message": "Cart updated successfully"}), 200
    except Exception as e:
        db.session.rollback()
        print(f"[ERROR] Exception occurred in update_cart_quantity: {e}")
...
def select_checkout_items():
    try:
        ...
        session['checkout_items'] = {item: int(request.form[f'quantity_{item}']) for item in selected_items}
        print(f"[DEBUG] Selected checkout items: {session['checkout_items']}")
        return redirect(url_for('user_interface.checkout'))
    except Exception as e:
        print(f"[ERROR] Exception occurred in select_checkout_items: {e}")
...
def checkout():
    ...
    if request.method == 'POST':
        ...
        try:
            customer.cart.checkout(checkout_items)
    ...
    cart_items = customer.cart.get_checkout_details(checkout_items)
    total_amount = customer.cart.calculate_total_for_items(checkout_items)

    return render_template('user/customer/checkout.html', cart_items=cart_items, total_amount=total_amount)
...
def select_payment_method():
    ...
    return render_template('user/customer/select_payment_method.html', total_amount=total_amount)
...
def pay_with_wallet():
    ...
    return render_template('user/customer/pay_with_wallet.html', total_amount=total_amount, wallet_balance=customer.wallet_balance)
