    ...
    try:
        ...
        if not item_orders:
            ...
            for order in item_orders:
                logging.debug("Order ID: %s, Payment Status: %s, Total Amount: %.2f",
                              order.id, order.payment_status, order.total_amount)
...
def checkout():
    try:
        ...
        for prod_round_id_str, qty in checkout_items.items():
            ...
            if cart_item:
                logging.debug("Processing CartItem: %s", cart_item)
        ...
        logging.debug("Calculated subtotal: %f, tax: %f, transaction fee: %f, total amount: %f",
                      subtotal, tax, transaction_fee, total_amount)

        # Check for existing unpaid orders for this session
        existing_order = ItemOrder.query.filter_by(
            customer_id=customer.id,
            payment_status="unpaid"
        ).first()

        if existing_order:
            logging.info("Unpaid order already exists. Reusing Order ID: %s", existing_order.id)
            item_order = existing_order
        else:
            logging.info("Creating a new unpaid ItemOrder.")
        ...
        if request.method == 'POST':
            ...
            logging.debug("Payment method selected: %s", payment_method)
            if payment_method == "wallet":
                logging.info("Redirecting to wallet payment for Order ID: %s", item_order.id)
                return redirect(url_for('user_interface.item_order_wallet_payment', order_id=item_order.id))
            else:
...
@user_interface.route('/item_order_wallet_payment/<uuid:order_id>', methods=['GET', 'POST'])
def item_order_wallet_payment(order_id):
    try:
        logging.debug("Entered /item_order_wallet_payment route with Order ID: %s", order_id)
        ...
        # Retrieve the existing ItemOrder
        item_order = ItemOrder.query.get(order_id)
        if not item_order or item_order.customer_id != customer.id:
            logging.error("Invalid or unauthorized order. Order ID: %s", order_id)
            flash("Invalid or unauthorized order.", "error")
            return redirect(url_for('user_interface.view_cart'))

        logging.info("Retrieved ItemOrder: %s with payment status: %s", item_order.id, item_order.payment_status)

        if request.method == 'POST':
            if customer.wallet_balance < item_order.total_amount:
                logging.warning("Insufficient wallet balance for Order ID: %s", order_id)
            ...
            logging.info("Processing payment for Order ID: %s", order_id)
            ...
            db.session.commit()
            logging.info("Payment successful for Order ID: %s", order_id)

            # Validate post-commit status
            updated_order = ItemOrder.query.get(order_id)
            logging.info("Post-commit status of Order ID %s: %s", order_id, updated_order.payment_status)
...
@user_interface.route('/pay_unpaid_item_order/<uuid:order_id>', methods=['GET', 'POST'])
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if payment_method == 'wallet':
                ...
                flash("Payment method not supported yet.", "error")
                return redirect(url_for('user_interface.pay_unpaid_order', order_id=order_id))
...
@user_interface.route('/update_artwork/<uuid:artwork_id>', methods=['GET', 'POST'])
