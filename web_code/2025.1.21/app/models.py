...
class ProductionRound(db.Model):
    ...
    delivery_order_items = db.relationship("DeliveryOrderItem", back_populates="production_round")
...
class ItemOrderItem(db.Model):
    ...
    item_status = db.Column(db.String(50), default="item")  # Tracks the current stage:"item", "refunded","transferred","delivered"
...
class DeliveryOrderItem(db.Model):
    ...
    __table_args__ = (db.Index('ix_delivery_order_item_package_id_item_id', 'delivery_package_id', 'item_order_item_id'),)

    delivery_package_id = db.Column(db.String(100), db.ForeignKey('delivery_packages.package_number'), primary_key=True)
    item_order_item_id = db.Column(UUID(as_uuid=True), db.ForeignKey('item_order_items.id'), primary_key=True)  # Direct link to ItemOrderItem
    production_round_id = db.Column(UUID(as_uuid=True), db.ForeignKey('production_rounds.id'), nullable=False, index=True)  # Link to ProductionRound
    ...
    item_order_item = db.relationship("ItemOrderItem", backref="deliveries")
    production_round = db.relationship("ProductionRound", back_populates="delivery_order_items")
