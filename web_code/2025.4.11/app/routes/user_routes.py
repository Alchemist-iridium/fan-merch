...
def signup():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.rollback()  # Roll back on error to maintain database integrity
...
# artwork and artwork collection
@user_interface.route('/artwork/<uuid6:artwork_id>')
def artwork_page(artwork_id):
    artwork = Artwork.query.get_or_404(artwork_id)

    customer = None
    unread_count = 0  # Default for unregistered users
    fallback_url = url_for('user_interface.home')  # Default for unregistered users

    # Check if the user is authenticated and a customer
    if current_user.is_authenticated and current_user.role == 'customer':
        customer = current_user
        fallback_url = url_for('user_interface.customer_home')
        # Calculate unread notifications only for customers
        unread_count = Notification.query.filter_by(
            user_id=customer.id,
            is_read=False
        ).count()

    # No need for explicit role restriction here since Flask-Login and role checks handle access
    # Unregistered users (not authenticated) will proceed with unread_count = 0
    ...
    collections = artwork.artwork_collections
...
@user_interface.route('/artwork_collection_page/<uuid6:collection_id>')
def artwork_collection_page(collection_id):
    collection = ArtworkCollection.query.get_or_404(collection_id)

    # Set fallback URL based on user authentication and role
    fallback_url = url_for('user_interface.home')  # Default for unauthenticated users
    unread_count = 0  # Default for unregistered users

    if current_user.is_authenticated and current_user.role == 'customer':
        fallback_url = url_for('user_interface.customer_home')
        # Calculate unread notifications for the customer
        unread_count = Notification.query.filter_by(
    ...
    return render_template(
        'user/public_search/artwork_collection_page.html',
...
# product and product collection

@user_interface.route('/product_page/<uuid6:product_id>', methods=['GET'])
def product_page(product_id):
    ...
    if current_user.is_authenticated and current_user.role == 'customer':
        ...
        # Check if the customer is signed up for notifications
    ...
    collections = product.product_collections  # Assumes a relationship is defined in the Product model

    return render_template(
        'user/public_search/product_page.html',
        product=product,
        ...
        fallback_url=fallback_url,
        collections=collections,
        unread_count=unread_count  # Pass unread_count to the template
    )



@user_interface.route('/product_collection/<uuid6:collection_id>', methods=['GET'])
def product_collection_page(collection_id):
    collection = ProductCollection.query.get_or_404(collection_id)
    ...
    return render_template(
        'user/public_search/product_collection_page.html',
        collection=collection,
        fallback_url=fallback_url,
        unread_count=unread_count
    )
...
def artist_public_page(artist_id):
    ...
    # Fetch artist's artwork collections
    artwork_collections = ArtworkCollection.query.filter_by(artist_id=artist.id).all()

    # Fetch artist's product collections
    product_collections = ProductCollection.query.filter_by(artist_id=artist.id).all()
    ...
    return render_template(
        ...
        customer=customer,
        fallback_url=fallback_url,
        unread_count=unread_count  # Pass unread_count for notifications
    )
...
def register_notification(product_id):
    ...
    if not customer:
        ...
        return redirect(url_for('user_interface.product_page', product_id=product_id))
...
# preview and create the delivery order here
...
def view_delivery_order_detail(order_id):
    ...
    production_round_details = {
        pr.id: {
            ...
            "artwork_image": pr.product.image_path if pr.product.image_path else pr.product.artwork.image_path
        }
...
def add_to_cart(round_id):
    try:
        ...
        if not customer.cart:
            customer.cart = Cart(customer_id=customer.id)
            db.session.add(customer.cart)
            db.session.commit()

        # 5) Add item(s) to the cart
...
@user_interface.route('/add_order_to_cart/<uuid6:order_id>', methods=['POST'])
@customer_required
...
# artwork collection management for approved artwork





@user_interface.route('/artwork_collections', methods=['GET'])
@login_required
def artwork_collections():
    artist_id = current_user.id
    collections = ArtworkCollection.query.filter_by(artist_id=artist_id).options(
        joinedload(ArtworkCollection.artworks)
    ).all()
    return render_template('user/artist/collections/artwork_collections.html', collections=collections)



@user_interface.route('/artwork_collections/<uuid6:collection_id>', methods=['GET'])
@artist_required
def view_artwork_collection(collection_id):
    
    collection = ArtworkCollection.query.get_or_404(collection_id)
    if collection.artist_id != current_user.id:
        flash('Unauthorized access.')
        return redirect(url_for('user_interface.artwork_collections'))
    return render_template('user/artist/collections/view_artwork_collection.html', collection=collection)



@user_interface.route('/artwork_collections/new', methods=['GET', 'POST'])
@artist_required
def new_artwork_collection():
    if request.method == 'POST':
        name = request.form['name']
        description = request.form.get('description', '')
        artwork_ids_str = request.form.getlist('artworks')  # List of UUID strings
        ...
        try:
            artwork_ids = [to_uuid(art_id, "artwork ID") for art_id in artwork_ids_str]
        except ValueError:
            flash('Invalid artwork ID format.')
            return redirect(request.url)

        collection = ArtworkCollection(name=name, description=description, artist_id=current_user.id)
        db.session.add(collection)
        db.session.commit()

        artworks = Artwork.query.filter(
            Artwork.id.in_(artwork_ids),
            Artwork.approval_status == 'Approved',
            Artwork.artist_id == current_user.id
        ).all()
        collection.artworks.extend(artworks)
        db.session.commit()

        flash('Collection created successfully.')
        return redirect(url_for('user_interface.artwork_collections'))

    approved_artworks = Artwork.query.filter_by(artist_id=current_user.id, approval_status='Approved').all()
    return render_template('user/artist/collections/new_artwork_collection.html', artworks=approved_artworks)




@user_interface.route('/artwork_collections/<uuid6:collection_id>/edit', methods=['GET', 'POST'])
@artist_required
def edit_artwork_collection(collection_id):
    collection = ArtworkCollection.query.get_or_404(collection_id)
    if collection.artist_id != current_user.id:
        flash('Unauthorized access.')
        return redirect(url_for('user_interface.artwork_collections'))
    ...
    if request.method == 'POST':
        name = request.form['name']
        description = request.form.get('description', '')
        artwork_ids_str = request.form.getlist('artworks')
        ...
        collection.artworks = artworks  # Replace existing artworks
        db.session.commit()
    ...
    approved_artworks = Artwork.query.filter_by(artist_id=current_user.id, approval_status='Approved').all()
    selected_ids = [str(artwork.id) for artwork in collection.artworks]
    return render_template('user/artist/collections/edit_artwork_collection.html',
                          collection=collection, artworks=approved_artworks, selected_ids=selected_ids)





@user_interface.route('/artwork_collections/<uuid6:collection_id>/delete', methods=['POST'])
@artist_required
def delete_artwork_collection(collection_id):
...
# product collection management for approved product



@user_interface.route('/product_collections', methods=['GET'])
@login_required
def product_collections():
    artist_id = current_user.id
    collections = ProductCollection.query.filter_by(artist_id=artist_id).options(
        joinedload(ProductCollection.products)
    ).all()
    return render_template('user/artist/collections/product_collections.html', collections=collections)




@user_interface.route('/product_collections/<uuid6:collection_id>', methods=['GET'])
@artist_required
def view_product_collection(collection_id):
    # collection_id is already a UUID object from the uuid6 converter
    collection = ProductCollection.query.options(
        joinedload(ProductCollection.products).joinedload(Product.artwork)
    ).get_or_404(collection_id)
    
    if collection.artist_id != current_user.id:
        flash('Unauthorized access.')
        return redirect(url_for('user_interface.product_page_collections'))
    
    return render_template('user/artist/collections/view_product_collection.html', collection=collection)





@user_interface.route('/product_collections/new', methods=['GET', 'POST'])
@artist_required
def new_product_collection():
    if request.method == 'POST':
        name = request.form['name']
        description = request.form.get('description', '')
        product_ids_str = request.form.getlist('products')
        ...
        try:
            product_ids = [to_uuid(prod_id, "product ID") for prod_id in product_ids_str]
        except ValueError:
            flash('Invalid product ID format.')
            return redirect(request.url)

        collection = ProductCollection(name=name, description=description, artist_id=current_user.id)
        db.session.add(collection)
        db.session.commit()
    ...
    approved_products = Product.query.filter_by(artist_id=current_user.id, production_status='Approved').all()
    return render_template('user/artist/collections/new_product_collection.html', products=approved_products)
...
@user_interface.route('/product_collections/<uuid6:collection_id>/edit', methods=['GET', 'POST'])
@artist_required
def edit_product_collection(collection_id):
    collection = ProductCollection.query.get_or_404(collection_id)
    if collection.artist_id != current_user.id:
        flash('Unauthorized access.')
        return redirect(url_for('user_interface.product_page_collections'))
    ...
    if request.method == 'POST':
        ...
        collection.products = products  # Replace existing products
        db.session.commit()
    ...
    approved_products = Product.query.filter_by(artist_id=current_user.id, production_status='Approved').all()
    selected_ids = [str(product.id) for product in collection.products]
    return render_template('user/artist/collections/edit_product_collection.html',
                          collection=collection, products=approved_products, selected_ids=selected_ids)





@user_interface.route('/product_collections/<uuid6:collection_id>/delete', methods=['POST'])
@artist_required
...
@user_interface.route('/update_artwork/<uuid6:artwork_id>', methods=['GET', 'POST'])  # Adjusted uuid6 to uuid
@artist_required
