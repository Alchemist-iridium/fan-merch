...
def create_app():
    ...
    app.config['UPLOAD_FOLDER'] = os.path.join(app.root_path, 'static/uploads') # for artwork image
    app.config['PRODUCT_UPLOAD_FOLDER'] = os.path.join(app.root_path, 'static/product_images')  # New config for products
    ...
    if not os.path.exists(app.config['PRODUCT_UPLOAD_FOLDER']):
        os.makedirs(app.config['PRODUCT_UPLOAD_FOLDER'])
