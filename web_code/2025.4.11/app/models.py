...
artwork_collection_membership = db.Table(
    'artwork_collection_membership',
    db.Column('artwork_id', UUID(as_uuid=True), db.ForeignKey('artworks.id'), primary_key=True),
    db.Column('collection_id', UUID(as_uuid=True), db.ForeignKey('artwork_collections.id'), primary_key=True)
)

product_collection_membership = db.Table(
    'product_collection_membership',
    db.Column('product_id', UUID(as_uuid=True), db.ForeignKey('products.id'), primary_key=True),
    db.Column('collection_id', UUID(as_uuid=True), db.ForeignKey('product_collections.id'), primary_key=True)
)
...
class Artist(User):
    ...
    artworks = db.relationship("Artwork", back_populates="artist", lazy=True, foreign_keys="[Artwork.artist_id]")
    ...
    products = db.relationship("Product", back_populates="artist", lazy=True, foreign_keys="[Product.artist_id]")
    ...
    # Fixed relationships with foreign_keys
    product_collections = db.relationship(
        "ProductCollection",
        back_populates="artist",
        foreign_keys="[ProductCollection.artist_id]",
        lazy=True
    )
    artwork_collections = db.relationship(
        "ArtworkCollection",
        back_populates="artist",
        foreign_keys="[ArtworkCollection.artist_id]",
        lazy=True
    )
...
class Customer(User):
    ...
    delivery_orders = db.relationship("DeliveryOrder", back_populates="customer", lazy="dynamic")
...
class Artwork(db.Model):
    ...
    manufacturing_specs = db.Column(db.String(50), nullable=True)
    hard_tags = db.Column(db.String(50), nullable=False)
    tag_approvals = db.Column(JSON, default=lambda: {}, nullable=False)  # New field for tag approvals
    soft_tags = db.Column(db.String(150), nullable=True)
    ...
    # Add relationship to artwork collections
    artwork_collections = db.relationship(
        "ArtworkCollection",
        secondary=artwork_collection_membership,
...
class ArtworkCollection(db.Model):
    ...
    artist = db.relationship("Artist", back_populates="artwork_collections", foreign_keys=[artist_id])
    artworks = db.relationship(
        "Artwork",
        secondary=artwork_collection_membership,
        back_populates="artwork_collections"
    )
...
class Product(db.Model):
    ...
    product_collections = db.relationship(
        "ProductCollection",
        secondary=product_collection_membership,
...
class ProductCollection(db.Model):
    ...
    artist = db.relationship("Artist", back_populates="product_collections", foreign_keys=[artist_id])
...
class ProductionRound(db.Model):
    ...
    admin = db.relationship("ProductionRoundAdmin", back_populates="production_rounds", foreign_keys=[admin_id])
...
class CartItem(db.Model):
    ...
    def populate_snapshot(self, production_round):
        ...
        self.product_image_path = production_round.product.image_path if production_round.product.image_path else production_round.product.artwork.image_path
...
class Payout(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid6)
    artist_id = db.Column(UUID(as_uuid=True), db.ForeignKey('users.id'), nullable=False, index=True)
    ...
    # Relationships
    artist = db.relationship("Artist", back_populates="payouts", foreign_keys=[artist_id])
