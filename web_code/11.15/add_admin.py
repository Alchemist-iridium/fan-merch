...
from main import db, ArtworkApprovalAdmin, OrderManagementAdmin, SuperAdmin
...
def add_admin(name, email, password, role):
    with app.app_context():
        ...
        if ArtworkApprovalAdmin.query.filter_by(email=email).first() or \
           ...
           SuperAdmin.query.filter_by(email=email).first():
        ...
        if role == 'artwork_approval_admin':
            ...
            new_admin = SuperAdmin(name=name, email=email, password_hash=hashed_password, role=role)
        else:
            print(f"Invalid role '{role}' specified. Please choose from 'artwork_approval_admin', 'order_management_admin', or 'super_admin'.")
            return
...
if __name__ == '__main__':
    ...
    role = input("Enter the role for the new admin (artwork_approval_admin, order_management_admin, super_admin): ").strip().lower()
