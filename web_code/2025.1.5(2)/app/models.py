...
class Customer(User):
    ...
    def create_item_order(self, cart_items, payment_method):
        ...
        item_order = ItemOrder(
            ...
            status="pending",         # or "cart", depending on your logic
    ...
    def _pay_for_item_order(self, item_order, total_amount, payment_method):
        ...
        item_order.status = "paid"  # overall order status
        session.add(item_order)
...
# ItemOrder can function as either a “cart” or a “completed order,” depending on its status.
# It holds references to the customer who is placing the order and an overall total or status.
...
class ItemOrder(db.Model):
    ...
    status = db.Column(Boolean, default=False)  # e.g., False = 'pending_payment', True = 'paid'
