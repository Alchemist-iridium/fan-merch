...
@user_interface.route('/update_product_status/<uuid:product_id>', methods=['POST'])
...
@user_interface.route('/admin_assign_product/<uuid:product_id>', methods=['GET', 'POST'])
def admin_assign_product(product_id):
    if 'user_id' not in session or session['role'] != 'product_approval_admin':
        flash("You need to be logged in as a Product Approval Admin to assign a product.")
        return redirect(url_for('user.login'))

    product = Product.query.get_or_404(product_id)

    if not product or product.production_status != 'Approved':
        flash("Invalid product or product is not approved for assignment.")
        return redirect(url_for('user.product_approval_admin_dashboard'))
    ...
    if request.method == 'POST':
        ...
        if admin_id and admin_id.isdigit():
            admin = OrderManagementAdmin.query.get(int(admin_id))
            if admin:
                product.assigned_admin_id = admin.id
                db.session.commit()
                flash(f"Product '{product.name}' has been assigned to admin '{admin.name}'.")
                return redirect(url_for('user.product_approval_admin_dashboard'))
            else:
                flash("Invalid admin selection. Please try again.")
        else:
            flash("Admin selection is required.")

    # Fetch order management admins for assignment
    admins = OrderManagementAdmin.query.all()

    return render_template('admin_assign_product.html', product=product, admins=admins)
