...
def add_to_cart(round_id):
    if 'user_id' not in session or session['role'] != 'customer':
        # Redirect to the login_required page with the appropriate action
...
def edit_product(product_id):
    ...
    return render_template('user/artist/edit_product.html', product=product, flash_message=flash_message)
...
def request_production_round(product_id):
    ...
    try:
        ...
        if product_admin:
            ...
            Notification.send_notification(user_id=product.assigned_admin_id, message=message,type='product')
...
# View Notifications for Artists and Customers
