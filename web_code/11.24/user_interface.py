...
@user_interface.route('/request_production_round/<uuid:product_id>', methods=['POST', 'GET'])
def request_production_round(product_id):
    ...
    try:
        artist_id = uuid.UUID(session['user_id'])
    except ValueError:
        flash("Invalid user ID format.")
        return redirect(url_for('user.login'))

    product = Product.query.get(product_id)
    if not product or product.artist_id != artist_id:
        flash("Unauthorized access or invalid product.")
        return redirect(url_for('user.artist_dashboard'))

    # Create a request to notify the assigned order_management_admin
    if not product.assigned_admin_id:
        return redirect(url_for('user.production_round_confirmation', product_id=product.id, success=False))

    order_admin = OrderManagementAdmin.query.get(product.assigned_admin_id)
    if not order_admin:
        return redirect(url_for('user.production_round_confirmation', product_id=product.id, success=False))

    # Flash message to simulate a request creation (this should be replaced with an actual request to the admin)
    flash(f"Production round request for product '{product.name}' has been submitted.")

    # Redirect to the confirmation page
    return redirect(url_for('user.production_round_confirmation', product_id=product.id, success=True))



@user_interface.route('/production_round_confirmation/<uuid:product_id>', methods=['GET'])
def production_round_confirmation(product_id):
    if 'user_id' not in session or session['role'] != 'artist':
        flash("You need to be logged in as an Artist to view this confirmation.")
        return redirect(url_for('user.login'))
    ...
    # Check if the confirmation was a success or failure based on the query parameter
    success = request.args.get('success', 'false').lower() == 'true'

    return render_template('production_round_confirmation_artist.html', product=product, success=success)
...
@user_interface.route('/initiate_production_round/<uuid:product_id>', methods=['GET', 'POST'])
def initiate_production_round(product_id):
    if 'user_id' not in session or session['role'] != 'order_management_admin':
        flash("You need to be logged in as an Order Management Admin to initiate a production round.")
        return redirect(url_for('user.login'))

    product = Product.query.get(product_id)
    if not product or product.assigned_admin_id != uuid.UUID(session['user_id']):
        flash("Unauthorized access or invalid product.")
        return redirect(url_for('user.order_management_admin_dashboard'))
    ...
    if request.method == 'POST':
        ...
        earnest_money = float(request.form['earnest_money'])
        min_production_size = int(request.form['min_production_size'])
        max_waiting_time = int(request.form['max_waiting_time'])

        # Create the production round
        new_round = ProductionRound(
            product_id=product.id,
            artist_id=product.artist_id,
            admin_id=product.assigned_admin_id,
            price_per_unit=price_per_unit,
            earnest_money=earnest_money,
            min_production_size=min_production_size,
            max_waiting_time=max_waiting_time
        )
        db.session.add(new_round)
        db.session.commit()

        flash(f"Production round for product '{product.name}' has been initiated.")
        return redirect(url_for('user.order_management_admin_dashboard'))

    return render_template('initiate_production_round.html', product=product)
...
def manage_production_round(round_id):
    ...
    if not production_round or production_round.admin_id != session['user_id']:
        flash("Unauthorized action.")
        return redirect(url_for('user.order_management_admin_dashboard'))

    if request.method == 'POST':
        # Update the production round details
        production_round.price_per_unit = float(request.form['price_per_unit'])
        production_round.earnest_money = float(request.form['earnest_money'])
...
# should be production round...this will be modified later
@user_interface.route('/update_product/<uuid:product_id>', methods=['GET', 'POST'])
def update_product(product_id):
    if 'user_id' not in session or session.get('role') != 'order_management_admin':
        flash("You need to be logged in as an Order Management Admin to update product information.")
        return redirect(url_for('user.login'))
    ...
    # Retrieve the product and ensure it's assigned to the logged-in admin
    product = Product.query.get(product_id)
    if not product or product.assigned_admin_id != admin_id:
...
def assign_order_management_admin(product_id):
    ...
    # Ensure the product_id is treated as a UUID object
    product = Product.query.get(product_id)
    ...
    if request.method == 'POST':
        ...
        # Parse admin_id to UUID if it exists
        if admin_id:
            try:
                admin_uuid = UUID(admin_id)  # Convert the admin_id to a UUID object
...
def add_to_cart(round_id):
    # Ensure that the user is logged in as a customer
    if 'user_id' not in session or session.get('role') != 'customer':
        flash("You need to be logged in as a Customer to add items to the cart.")
        return redirect(url_for('user.login'))
    ...
    try:
        production_round_uuid = UUID(str(round_id))
    except ValueError:
        flash("Invalid production round ID.")
        return redirect(url_for('user.home'))

    # Fetch the production round
    production_round = ProductionRound.query.get(production_round_uuid)
    ...
    try:
        ...
        return redirect(url_for('user.production_round_page', round_id=production_round.id))

    # Check if the customer already has this round in the cart
    existing_order = Order.query.filter_by(customer_id=customer_id, production_round_id=production_round.id, status='pending').first()
    if existing_order:
        # Update the quantity in the cart
        existing_order.quantity += quantity
        flash(f"Updated quantity of Product '{production_round.product.name}' in the cart.")
    else:
        ...
        new_order = Order(
            ...
            earnest_payment_date=None,
            final_payment_date=None
        )
        new_order.quantity = quantity
        db.session.add(new_order)
        flash(f"Added '{production_round.product.name}' to the cart.")
...
# dialog function between the artist and the order management admin
...
def dialog(round_id):
    ...
    if session['role'] == 'artist' and production_round.artist_id != uuid.UUID(session['user_id']):
        flash("Unauthorized access to this dialog.")
        return redirect(url_for('user.artist_dashboard'))

    if session['role'] == 'order_management_admin' and production_round.admin_id != uuid.UUID(session['user_id']):
        flash("Unauthorized access to this dialog.")
        return redirect(url_for('user.order_management_admin_dashboard'))
