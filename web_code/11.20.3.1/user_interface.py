...
from models import User, Artist, Customer, Artwork, Product, ArtworkApprovalAdmin, OrderManagementAdmin, ProductApprovalAdmin, DesignFile
...
@user_interface.route('/view_product_disapproval_reason/<int:product_id>')
def view_product_disapproval_reason(product_id):
    ...
    if not product or product.production_status != 'Disapproved' or product.artwork.artist_id != session['user_id']:
...
def artwork_approval_admin_dashboard():
    ...
    if request.method == 'POST':
        ...
        # Handle artwork disapproval by redirecting to the disapproval page
...
@user_interface.route('/approve_artwork/<int:artwork_id>', methods=['POST'])
...
def product_approval_admin_dashboard():
    ...
    if request.method == 'POST':
        # Approve Product
        if 'approve_id' in request.form:
            product_id = request.form['approve_id']
            product = Product.query.get(product_id)
            if product:
                # Set product status to approved
                product.production_status = 'Approved'

                # Assign the product to an Order Management Admin
                # (This is a simplified version; you might want to add more logic to select the admin)
                order_management_admin = OrderManagementAdmin.query.first()  # Example of assigning the first available admin
                if order_management_admin:
                    product.assign_admin(order_management_admin.id)
                    flash(f"Product '{product.name}' approved and assigned to Order Management Admin.")
                    db.session.commit()
                else:
                    flash("No Order Management Admin available for assignment.")

        # Handle disapproval request
        elif 'disapprove_id' in request.form:
            product_id = request.form['disapprove_id']
...
@user_interface.route('/disapprove_product/<int:product_id>', methods=['GET', 'POST'])
def disapprove_product(product_id):
    # Verify that user is logged in and is a Product Approval Admin
    if 'user_id' not in session or session['role'] != 'product_approval_admin':
        flash("You need to be logged in as a Product Approval Admin to disapprove a product.")
        return redirect(url_for('user.login'))

    # Fetch the product based on product_id and ensure it is in Pending state
    product = Product.query.get(product_id)
    if not product or product.production_status != 'Pending':
        flash("Invalid product or product is not pending approval.")
        return redirect(url_for('user.product_approval_admin_dashboard'))

    if request.method == 'POST':
        reason = request.form.get('disapproval_reason')
        if reason:
            # Use the Product class method to handle disapproval and commit changes
            product.disapprove(reason)
            flash(f"Product '{product.name}' has been disapproved with the given reason.")
            return redirect(url_for('user.product_approval_admin_dashboard'))
        else:
...
# Route to disapprove an artwork with a detailed reason
@user_interface.route('/disapprove_artwork/<int:artwork_id>', methods=['GET', 'POST'])
def disapprove_artwork(artwork_id):
    ...
    if request.method == 'POST':
        # Get the disapproval reason from the form
        reason = request.form['disapprove_reason']

        # Update artwork status to 'Disapproved' with the reason
    ...
    # Render the disapproval reason form if the method is GET
    return render_template('disapprove_artwork.html', artwork=artwork)




@user_interface.route('/view_artwork_disapproval_reason/<int:artwork_id>')
def view_artwork_disapproval_reason(artwork_id):
    ...
    if not artwork or artwork.artist_id != session['user_id']:
        flash("You are not authorized to view this disapproval reason.")
        return redirect(url_for('user.artist_dashboard'))
...
def admin_assign_product(product_id):
    if 'user_id' not in session or session.get('role') != 'product_approval_admin':
        return redirect(url_for('user.login'))
