...
class ProductionRound(db.Model):
    ...
    def refund_items_if_abandoned(production_round_id):
        """
        ...
        """
        try:
            # 1) Fetch the production round
            production_round = ProductionRound.query.get(production_round_id)
            if not production_round:
                print(f"[DEBUG] ProductionRound {production_round_id} not found.")
                return

            if production_round.price is None:
                print("[DEBUG] ProductionRound price is None—cannot proceed with refunds.")
                return

            # 2) Query items with status='item'
            items_to_refund = (
                ItemOrderItem.query
                .filter_by(
            ...
            if not items_to_refund:
                print("[DEBUG] No items in 'item' status to refund.")
                return

            print(f"[DEBUG] Found {len(items_to_refund)} items to refund for round {production_round_id}")

            # 3) Create RefundItemOrder for each item
            from datetime import datetime
            refund_records = []
            for item in items_to_refund:
                customer_id = item.item_order.customer_id
                refund_amount = production_round.price

                # Build the RefundItemOrder
                refund_order = RefundItemOrder(
                    customer_id=customer_id,
                    item_order_item_id=item.id,
                    refund_amount=refund_amount,
                    is_auto=True,
                    reason="auto-full-refund",
            ...
            # 6) Sum refunds by customer, update wallets
            from collections import defaultdict
            refunds_by_customer = defaultdict(float)
            for r_record in refund_records:
                refunds_by_customer[r_record.customer_id] += r_record.refund_amount

            for cust_id, total_refund in refunds_by_customer.items():
                cust_obj = Customer.query.get(cust_id)
                if cust_obj:
                    old_bal = cust_obj.wallet_balance
                    cust_obj.wallet_balance = old_bal + total_refund

            # 7) Commit #2
            db.session.commit()

            print(f"[DEBUG] Refund process completed. {len(items_to_refund)} items refunded.")

        except Exception as e:
            db.session.rollback()
            print(f"[ERROR] Refund process failed: {str(e)}")
...
class Cart(db.Model):
    ...
    # Store cart items as JSON dictionary (production_round_id: quantity)
    cart_items = db.Column(JSON, nullable=False, default=dict)  
...
class ItemOrder(db.Model):
    ...
    #region record, to handle tax calculation and better handle multi-warehouse scenario
    ...
    def validate_production_round_status(self):
        ...
        for item in self.item_list:
            ...
            if not production_round or production_round.stage != 'waiting':
                expired_items.append(f"Production round {item['production_round_id']} is expired.")

        if expired_items:
            raise ValueError("; ".join(expired_items))
    ...
    def mark_as_paid(self, payment_method: str, payment_reference: str = None, region=None):
        ...
        self.register_customer_for_notifications()
        ...
        for item in self.item_list:
            for _ in range(item['quantity']):
                active_item = ActiveItem(
                    id=uuid6(),
                    item_order_id=self.id,  # Stored as a record, no automatic relationship
                    production_round_id=uuid.UUID(item['production_round_id']),
                    region=region,  # Assign selected region
                    unit_price=item.get('unit_price', 0),
                    product_name=item.get('product_name', "Unknown"),
                    product_image_path=item.get('product_image_path', None),
                )
    ...
    def register_customer_for_notifications(self):
        ...
        for item in self.item_list:
            ...
            notification = ProductionRoundNotification.query.filter_by(
                production_round_id=production_round_id, customer_id=customer_id
            ).first()

            if notification:
                notification.quantity += item['quantity']
            else:
                new_notification = ProductionRoundNotification(
                    production_round_id=production_round_id, customer_id=customer_id, quantity=item['quantity']
                )
...
class ItemOrderItem(db.Model):
    ...
    __mapper_args__ = {
        'polymorphic_identity': 'item_order_item',
        'polymorphic_on': type
    }
    ...
    def __repr__(self):
        return f"<ItemOrderItem {self.id} order={self.item_order_id} status={self.item_status}>"




class ActiveItem(ItemOrderItem):
    """
    ...
    id = db.Column(UUID(as_uuid=True), db.ForeignKey('item_order_items.id'), primary_key=True)
    

    __mapper_args__ = {
        'polymorphic_identity': 'active_item',
    }

    @staticmethod
    def move_to_stocking(item_id):
        """
        Moves an ActiveItem to StockingItem when the production is completed.
        """
        item = db.session.query(ActiveItem).get(item_id)
        if not item:
            raise ValueError(f"ActiveItem with ID {item_id} not found.")

        stocking_item = StockingItem(
            id=item.id,
        ...
        db.session.delete(item)  # Remove from ActiveItem table
        db.session.add(stocking_item)  # Add to StockingItem table
        db.session.commit()
        return stocking_item





class StockingItem(ItemOrderItem):
    """
    Child model inheriting from ItemOrderItem for active items.
    These are items in stocking stage but not delivered to the customers
    """
    __tablename__ = "stocking_items"
    id = db.Column(UUID(as_uuid=True), db.ForeignKey('item_order_items.id'), primary_key=True)
    

    __mapper_args__ = {
        'polymorphic_identity': 'stocking_item',
    }

    @staticmethod
    def move_to_inactive(item_id, reason):
        """
        Moves a StockingItem to InactiveItem after delivery.
        """
        item = db.session.query(StockingItem).get(item_id)
        if not item:
            raise ValueError(f"StockingItem with ID {item_id} not found.")
        ...
        inactive_item = InactiveItem(
            ...
            product_image_path=item.product_image_path,
            region=item.region,
            reason=reason  # Typically "delivered"
        )

        db.session.delete(item)  # Remove from StockingItem table
        db.session.add(inactive_item)  # Add to InactiveItem table
        db.session.commit()
...
class InactiveItem(ItemOrderItem):
    """
    ...
    """
    __tablename__ = "inactive_items"
    id = db.Column(UUID(as_uuid=True), db.ForeignKey('item_order_items.id'), primary_key=True)
    reason= db.Column(db.String(50), default="item")  #item/refund/transfer/delivered

    __mapper_args__ = {
        'polymorphic_identity': 'inactive_item',
    }

    @staticmethod
    def move_from_active(item_id, reason):
        """
