...
def item_management(category):
    ...
    try:
        ...
        if category == "in_production":
            items = items_query.join(ProductionRound).filter(
                ProductionRound.is_active.is_(True),
                ProductionRound.stage.in_(["waiting", "sample", "production", "examination"])
            ).all()
        elif category == "waiting_for_delivery":
            items = items_query.join(ProductionRound).filter(
            ...
            items = items_query.filter(ItemOrderItem.item_status == "refunded").all()
        elif category == "transferred":
            items = items_query.filter(ItemOrderItem.item_status == "transferred").all()
        elif category == "all":
        ...
        logging.exception("Error in /item_management/<string:category>: %s", e)
        flash("An error occurred while loading items.")
        return redirect(url_for('user_interface.customer_dashboard'))
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if customer.wallet_balance < total_amount:
                logging.warning("Insufficient wallet balance for customer ID: %s", customer.id)
                db.session.commit()  # Save the unpaid order
...
@user_interface.route('/unpaid_item_order_wallet_payment/<uuid:order_id>', methods=['GET', 'POST'])
def unpaid_item_order_wallet_payment(order_id):
    try:
        ...
        logging.debug(f"Order Details: ID={item_order.id}, Total Amount={item_order.total_amount}, Payment Status={item_order.payment_status}")
        ...
        if request.method == 'POST':
            ...
            if customer.wallet_balance < item_order.total_amount:
                logging.warning(f"Insufficient wallet balance for customer ID: {customer_id}. Wallet Balance={customer.wallet_balance}, Order Total={item_order.total_amount}")
                flash("Insufficient wallet balance to complete the payment.", "error")
                return redirect(url_for('user_interface.pay_unpaid_item_order', order_id=order_id))
...
@user_interface.route('/archived_production_rounds/<uuid:product_id>', methods=['GET'])
...
@user_interface.route('/archived_production_round_dialogs/<uuid:round_id>', methods=['GET'])
...
@user_interface.route('/archived_production_round_details/<uuid:round_id>', methods=['GET'])
