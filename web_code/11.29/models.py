...
class Notification(db.Model):
    ...
    @classmethod
    def send_notification(cls, user_id, message, link=None):
    ...
    @classmethod
    def notify_new_product(cls, product):
        """Notify followers about a new product by an artist."""
        with current_app.app_context():
            try:
                # Assuming `followers` is a relationship of Artist that contains User objects.
                followers = product.artist.followers  
                link = url_for('user.artwork_public', artwork_id=product.artwork.id, _external=True)
                message = f"'{product.name}' has been added by {product.artist.name}."

                for follower in followers:
                    cls.send_notification(user_id=follower.id, message=message, link=link)
            except Exception as e:
                # Log the error if necessary, e.g. using current_app.logger
                current_app.logger.error(f"Error while notifying followers: {str(e)}")
