...
def view_delivery_orders():
    """
    Display all delivery orders categorized by their status (created, in_process, delivered).
    """
    if 'user_id' not in session or session['role'] != 'customer':
    ...
    try:
        ...
        categorized_orders = {
            ...
            "delivered": [order for order in delivery_orders if order.status == "delivered"]
        }
...
@user_interface.route('/view_delivery_order/<uuid6:order_id>', methods=['GET'])
def view_delivery_order_detail(order_id):
    """
    Display detailed information for a specific delivery order.
    """
    if 'user_id' not in session or session['role'] != 'customer':
        flash("You need to log in as a Customer to view your delivery order details.")
        return redirect(url_for('user_interface.login'))
    ...
    try:
        ...
        delivery_order = DeliveryOrder.query.filter_by(id=order_id, customer_id=customer.id).first()
        if not delivery_order:
            flash("Delivery order not found.")
        ...
        for item in delivery_order.delivery_item:
            ...
            if production_round_id not in grouped_items:
                production_round = ProductionRound.query.get(uuid.UUID(production_round_id))
                grouped_items[production_round_id] = {
                    "product_name": production_round.product.name,
                    "artwork_image": production_round.product.artwork.image_path,
                    "total_quantity": 0
                }
            grouped_items[production_round_id]["total_quantity"] += 1

        # Fetch packages
        packages = delivery_order.packages

        return render_template(
            'user/customer/view_delivery_order_detail.html',
            delivery_order=delivery_order,
            grouped_items=grouped_items.values(),  # Convert to a list for easier template rendering
            packages=packages
        )

    except Exception as e:
        logging.exception("Error in view_delivery_order_detail route: %s", e)
        flash(f"An error occurred while fetching the delivery order: {e}", "danger")
        return redirect(url_for('user_interface.view_delivery_orders'))
