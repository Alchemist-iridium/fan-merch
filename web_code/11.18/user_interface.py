...
def artist_dashboard():
    ...
    if request.method == 'POST':
        ...
        manufacturing_specs = request.form.get('manufacturing_specs')
        hard_tags_raw = request.form.get('hard_tags', "")
    ...
    artworks = artist.artworks  # Use the relationship to get all artworks for the artist
...
def submit_product(artwork_id):
    if 'user_id' not in session or session['role'] != 'artist':
        flash("You need to be logged in as an Artist to submit a product.")
        return redirect(url_for('user.login'))
    ...
    if request.method == 'POST':
        ...
        if production_specs not in valid_specs:
            flash("Invalid production specs. Must match artwork manufacturing specs.")
            return redirect(request.url)
        ...
        flash("Product submitted successfully for Super Admin approval.")
        return redirect(url_for('user.artist_dashboard'))
...
def edit_product(product_id):
    if 'user_id' not in session or session['role'] != 'artist':
        flash("You need to be logged in as an Artist to edit a product.")
        return redirect(url_for('user.login'))

    product = Product.query.get(product_id)
    if product is None or product.artwork.artist_id != session['user_id']:
        flash("You do not have permission to edit this product.")
        return redirect(url_for('user.artist_dashboard'))

    if request.method == 'POST':
        # Save product introduction text with HTML formatting
        product.introduction = request.form['introduction']
        ...
        if 'image' in request.files:
            ...
            if file and file.filename != '':
                ...
                file_path = os.path.join(current_app.config['UPLOAD_FOLDER'], filename)
                file.save(file_path)
                # Insert image reference into product introduction
                product.introduction += f'<img src="/{file_path}" alt="{filename}">'
        
        db.session.commit()
        flash("Product introduction updated successfully.")
        return redirect(url_for('user.artist_dashboard'))
