...
def item_management(category):
    ...
    try:
        ...
        category_filters = {
            "unpaid": items_query.filter(ItemOrderItem.item_status == "unpaid"),
...
def item_order_wallet_payment():
    try:
        ...
        if request.method == 'POST':
            # Create the ItemOrder with payment_status as "unpaid"
            item_order = ItemOrder(customer_id=customer.id, payment_status="unpaid", total_amount=total_amount)
            db.session.add(item_order)
            db.session.commit()  # Ensure the order is created

            # Call the new function to create ItemOrderItems
            item_order.create_item_order_items(checkout_items, customer.cart.id)
...
def auto_refund_item(item_id):
    ...
    try:
        ...
        product = Product.query.get(production_round.product_id)  # Ensure product exists
        artwork = Artwork.query.get(product.artwork_id) if product else None  # Fetch artwork safely
        ...
        # Debugging output
        logging.debug(f"Refund Page - Item ID: {item.id}, Refund Amount: {refund_amount}, Refund Type: {refund_type}")
        logging.debug(f"Product Name: {product.name}, Artwork: {artwork.title}")
        ...
        logging.exception("Error in auto_refund_item route: %s", e)
