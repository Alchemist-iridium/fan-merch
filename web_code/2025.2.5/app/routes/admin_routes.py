...
def confirm_production_round_update():
    try:
        ...
        if request.method == 'POST':
            ...
            # If in sample stage, also update the partial_refund amount
            if updated_data['stage'] == 'sample':
            ...
            # Check if the production round was abandoned and trigger refunds
            if updated_data['stage'] == "abandoned":
                ProductionRound.refund_items_if_abandoned(production_round.id)
...
def warehouse_admin_dashboard():
    ...
    if request.method == 'POST':
        ...
        # Check if the search query is a valid UUID
        try:
            uuid_query = uuid.UUID(search_query)
        except ValueError:
            # Not a valid UUID, fallback to searching by string
            uuid_query = None

        if search_query:
            production_rounds = ProductionRound.query.join(Product).filter(
                (ProductionRound.id == uuid_query) |  # Match by UUID
                (ProductionRound.product_id == uuid_query) |  # Match by UUID
                (Product.name.ilike(f"%{search_query}%")),  # Match by name
                ProductionRound.stage.in_(["production", "examination", "stocking"])  # Include only specific stages
            ).all()
    ...
    # Process production goals and total items ordered
...
@admin_interface.route('/warehouse_admin/storage/create', methods=['GET', 'POST'])
def create_warehouse_storage():
    ...
    if request.method == 'POST':
        ...
        if not location_name or not size or not warehouse:
            flash("Location name, size, and warehouse are required.", "danger")
            return redirect(url_for('admin_interface.create_warehouse_storage'))
    ...
    return render_template('admin/warehouse_admin/create_warehouse_storage.html')
...
def search_warehouse_storage():
    """Search for storage locations by location name, size, and warehouse."""
    if 'user_id' not in session or session['role'] != 'warehouse_admin':
        flash("You must log in as a Warehouse Admin to access this page.")
        return redirect(url_for('admin_interface.admin_login'))
    ...
    if warehouse:
        storages = storages.filter(WarehouseStorage.warehouse == int(warehouse))

    storages = storages.all()
    return render_template('admin/warehouse_admin/search_warehouse_storage.html', storages=storages)
...
def delete_warehouse_storage(storage_name):
    ...
    # Fetch the storage by location_name (not ID)
...
@admin_interface.route('/warehouse_admin/record/<uuid6:production_round_id>', methods=['GET', 'POST'])
def manage_warehouse_record(production_round_id):
    """Create or modify warehouse records for a production round."""
    if 'user_id' not in session or session['role'] != 'warehouse_admin':
        flash("You must log in as a Warehouse Admin to access this page.")
        return redirect(url_for('admin_interface.admin_login'))
    ...
    if request.method == 'POST':
        try:
            ...
            warehouse = int(request.form.get('warehouse', 0))
            quantity = int(request.form.get('quantity', 0))
            description = request.form.get('description', '')
            warehouse_admin_id = uuid.UUID(session['user_id'])

            # Select the first available storage location for the given size and warehouse
            ...
            if not storage_location:
                flash(f"No available storage location found for Size {size} in Warehouse {warehouse}.", "danger")
                return redirect(url_for('admin_interface.manage_warehouse_record', production_round_id=production_round.id))

            # Create a new warehouse record
            warehouse_record = WarehouseRecord(
                production_round_id=production_round.id,
                warehouse_storage_location=storage_location.location_name,  # Reference to storage location name
                quantity=quantity,
            ...
            # Mark the storage location as unavailable
            storage_location.is_available = False
            db.session.add(storage_location)

            # Update production round acceptance status
    ...
    # Fetch existing warehouse records for the production round
    warehouse_records = WarehouseRecord.query.filter_by(
        production_round_id=production_round.id
    ).all()
...
def warehouse_allocation_report_data(production_round_id):
    """Fetch warehouse allocation summary for AJAX call."""
    if 'user_id' not in session or session['role'] != 'warehouse_admin':
    ...
    # Mapping: Customer Region → Warehouse
    region_to_warehouse = {1: 1, 2: 1, 3: 2, 4: 3}

    # Fetch customer regions and item quantities
    customer_data = (
        db.session.query(
            Customer.region,
            ProductionRoundNotification.quantity
        )
        .join(Customer, Customer.id == ProductionRoundNotification.customer_id)
        .filter(ProductionRoundNotification.production_round_id == production_round_id)
        .all()
    )

    # Aggregate quantities by warehouse
    warehouse_summary = {1: 0, 2: 0, 3: 0}
    for region, quantity in customer_data:
        warehouse = region_to_warehouse.get(region)
        if warehouse:
            warehouse_summary[warehouse] += quantity
...
def delivery_admin_dashboard():
    """Dashboard for delivery admin to select warehouse and manage delivery orders."""
    if 'user_id' not in session or session['role'] != 'delivery_admin':
        flash("You must log in as a Delivery Admin to access this page.")
        return redirect(url_for('admin_interface.admin_login'))

    # Store warehouse selection in session to persist across page loads
    if request.method == 'POST':
        selected_warehouse = request.form.get('warehouse')
        if selected_warehouse:
            session['selected_warehouse'] = int(selected_warehouse)

    selected_warehouse = session.get('selected_warehouse')

    # Fetch delivery orders only if a warehouse is selected
    delivery_orders = []
    if selected_warehouse:
        region_to_warehouse = {1: 1, 2: 1, 3: 2, 4: 3}
        valid_regions = [region for region, warehouse in region_to_warehouse.items() if warehouse == selected_warehouse]

        delivery_orders = DeliveryOrder.query.filter(
            DeliveryOrder.region.in_(valid_regions),
...
def delivery_admin_workspace(warehouse):
    """Workspace to manage picked delivery orders and assign packages."""
    if 'user_id' not in session or session['role'] != 'delivery_admin':
        flash("You must log in as a Delivery Admin to access this page.")
        return redirect(url_for('admin_interface.admin_login'))

    # Get all "in_process" delivery orders for the selected warehouse
    region_to_warehouse = {1: 1, 2: 1, 3: 2, 4: 3}
    valid_regions = [region for region, wh in region_to_warehouse.items() if wh == warehouse]
    ...
    for order in delivery_orders:
        ...
        for item in order.delivery_item:
            production_round_id = uuid.UUID(item["production_round_id"])  # Convert to UUID
            grouped_items[production_round_id] = grouped_items.get(production_round_id, 0) + 1
            production_round_ids.add(production_round_id)
            order_production_rounds[order.id].append(production_round_id)  # Track PRs per order
    ...
    # Retrieve warehouse records for the selected warehouse
    warehouse_records = WarehouseRecord.query.filter(
        WarehouseRecord.production_round_id.in_(production_round_ids),
        WarehouseRecord.storage_location.has(warehouse=warehouse)
    ).all()
    ...
    return render_template(
        ...
        order_production_rounds=order_production_rounds  # Pass PRs per order
    )
...
def unpick_delivery_order(order_id):
    """Unpick a delivery order and set its status back to 'created'."""
    delivery_order = DeliveryOrder.query.get_or_404(order_id)
    ...
    return redirect(url_for('admin_interface.delivery_admin_workspace', warehouse=delivery_order.region))
...
def assign_delivery_package_view(order_id, warehouse):
    """
    GET-only route: displays Delivery Order info, Production Rounds, 
    and WarehouseRecords in the selected warehouse. 
    No database update is done here.
    """
    if 'user_id' not in session or session['role'] != 'delivery_admin':
        flash("You must log in as a Delivery Admin to access this page.")
        return redirect(url_for('admin_interface.admin_login'))
    ...
    # 2. Group items by production round (delivery_item is stored in a JSON column)
    grouped_items = {}
    ...
    # 3. Fetch ProductionRounds for display
    ...
    for pr in production_rounds:
        production_round_details[pr.id] = {
            "product_name": pr.product.name,
            "artwork_image": pr.product.artwork.image_path,
            "stage_goals": pr.stage_goals  # Example: [{"quantity": X, "gift": "..."}, ...]
        }

    # 4. Fetch WarehouseRecords that belong to the same warehouse + production rounds
    warehouse_records = WarehouseRecord.query.filter(
        WarehouseRecord.production_round_id.in_(production_round_ids),
        WarehouseRecord.storage_location.has(warehouse=warehouse)
    ).all()
...
def process_assign_delivery_package():
    ...
    Example:
      - Order PR needs: 5
      - Selected records for that PR:
          record1: qty=7 -> after deduction=2
          record2: qty=10 -> after deduction=5
          ...
      => We do this for *each* record in that PR.
    """
    ...
    try:
        ...
        # Then convert to UUID objects as needed and proceed with your deduction logic...
        ...
        #    e.g. if we have {"production_round_id": "...", "quantity": 5} in the JSON
        grouped_items = {}
        for item_data in delivery_order.delivery_item:
            pr_id = uuid.UUID(item_data["production_round_id"])
            qty = item_data.get("quantity", 1)
        ...
        #    in chosen_records that matches that round.
        for pr_id, needed_qty in grouped_items.items():
            ...
            # If no records selected for this PR, that's typically OK or might be an error
            # Decide if you want to require the user to pick records for each PR
            if not pr_records:
            ...
            for rec in pr_records:
                ...
                if rec.quantity < needed_qty:
                    # If it can't subtract that amount, we do a rollback and error out
                    db.session.rollback()
        ...
        # 8. After all records are deducted, update the DeliveryOrder status
