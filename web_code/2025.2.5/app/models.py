...
class ItemOrder(db.Model):
    ...
    def create_item_order_items(self, checkout_items, cart_id):
        """
        ...
        for prod_round_id_str, qty in checkout_items.items():
            production_round_id = uuid.UUID(prod_round_id_str)
            cart_item = CartItem.query.filter_by(cart_id=cart_id, production_round_id=production_round_id).first()
            if cart_item:
                for _ in range(qty):
                    item_order_item = ItemOrderItem(
                        ...
                        item_status="unpaid"  # Set status to "unpaid"
                    )
                    db.session.add(item_order_item)

        db.session.commit()  # Save all ItemOrderItem records
    ...
    # item is created as unpaid first, then it is updated after the corresponding order is successfully paid

    def update_item_status_to_paid(self):
        """Update all related ItemOrderItem statuses to 'item' after payment success."""
        for item in self.order_items:
...
class ItemOrderItem(db.Model):
    ...
    item_status = db.Column(db.String(50), default="item")  # Tracks the current stage:"unpaid","item", "in_process","refunded","transferred","delivered"
