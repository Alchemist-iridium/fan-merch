...
class Artist(User):
    ...
    def submit_work(self, work_id, title, description, manufacturing_specs, hard_tags, soft_tags=None):
        # Ensure at least one hard tag is provided
        if not hard_tags:
    ...
    def view_submitted_works(self):
        # Allows the artist to view their submitted works along with approval statuses
        return [
            {
...
class PlatformAdministrator(User):
    ...
    def reject_work(self, work):
        # Allows admins to reject a submitted work
        work['approval_status'] = 'Rejected'
        return f"Work {work['work_id']} has been rejected."
    ...
    def search_artworks(self, artworks, keyword):
        results = []
        for work in artworks:
            if (keyword.lower() in work['title'].lower() or
                keyword.lower() in work['description'].lower() or
                any(keyword.lower() in tag.lower() for tag in work['hard_tags'] + work['soft_tags'])):
                results.append(work)
        return results
