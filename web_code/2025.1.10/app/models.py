...
class Customer(User):
    ...
    def _pay_for_item_order(self, item_order, total_amount, payment_method):
        """
        Finalizes payment for an ItemOrder and transitions PreItems to Items.
        
        :param item_order: The ItemOrder instance to finalize payment for.
        :param total_amount: The total amount to be paid.
        :param payment_method: The method of payment (e.g., "wallet").
        """
        from sqlalchemy.orm import Session
        ...
        item_order.payment_method = payment_method
        session.add(item_order)

        # Transition PreItems to Items
        for order_item in item_order.order_items:
            if order_item.item_type == "preitem":
                pre_item = order_item.item
                new_item = pre_item.confirm_checkout()  # Convert PreItem to Item
                session.add(new_item)

                # Update ItemOrderItem to link to the new Item
...
class BaseItem(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)  # Consistent UUID across stages
    production_round_id = db.Column(UUID(as_uuid=True), db.ForeignKey('production_rounds.id'), nullable=False)
    customer_id = db.Column(UUID(as_uuid=True), db.ForeignKey('customers.id'), nullable=False)  # Consistent for all stages
...
class DeliveredItem(BaseItem):
    ...
    delivery_order_items = db.relationship(
        "DeliveryOrderItem",
        back_populates="delivered_item",
        cascade="all, delete-orphan"
    )
...
class AbandonedItem(BaseItem):
    ...
    refund_item_order = db.relationship(
        "RefundItemOrder",
...
class ItemOrderItem(db.Model):
    ...
    item_order_id = db.Column(UUID, db.ForeignKey('orders.id'), primary_key=True)
    production_round_id = db.Column(UUID, db.ForeignKey('production_rounds.id'), primary_key=True)
    item_id = db.Column(UUID, db.ForeignKey('base_items.id'), nullable=False, unique=True)
    item_type = db.Column(db.String(50), default="preitem")  # Tracks the current stage: "preitem", "item", "delivered", "abandoned"
    ...
    item = db.relationship("BaseItem", backref="order_item")  # Single `BaseItem` linked to one `ItemOrderItem`
...
class RefundItemOrder(db.Model):
    ...
    item_id = db.Column(UUID(as_uuid=True), db.ForeignKey("abandoned_items.id"), nullable=False, unique=True)

    # Relationships
    customer = db.relationship("Customer", back_populates="refund_item_orders")
    
    abandoned_item = db.relationship(
        "AbandonedItem",
        back_populates="refund_item_order"
    )
...
class TransferItemOrder(db.Model):
    ...
    # Link to either Item or DeliveredItem, linked with the base item
    ...
    # Explicit join to BaseItem using primaryjoin
    item = db.relationship(
        "BaseItem",
        primaryjoin="TransferItemOrder.item_id == BaseItem.id",
        backref="transfer_item_orders"
    )
