...
@user_interface.route('/list_orders/<string:order_type>')
def list_orders(order_type):
    ...
    try:
        ...
        if order_type == 'item_order':
            orders = customer.orders.all()
        elif order_type == 'transfer_order':
            orders = customer.transfer_item_orders_outgoing.all()
        elif order_type == 'refund_order':
            orders = customer.refund_item_orders.all()
        elif order_type == 'delivery_order':
            orders = customer.delivery_orders.all()
        else:
            flash("Invalid order type.")
            return redirect(url_for('user_interface.customer_dashboard'))

        return render_template('user/customer/order_list.html', orders=orders, order_type=order_type)
...
@user_interface.route('/order_details/<uuid:order_id>')
def order_details(order_id):
    if 'user_id' not in session or session['role'] != 'customer':
        flash("You need to be logged in as a Customer to view order details.")
        return redirect(url_for('user_interface.login'))
    ...
    try:
        ...
        order = ItemOrder.query.filter_by(id=order_id, customer_id=customer.id).first()

        if not order:
            flash("Order not found or unauthorized access.")
            return redirect(url_for('user_interface.customer_dashboard'))

        return render_template('user/customer/order_details.html', order=order)
...
def item_order_wallet_payment():
    try:
        ...
        # Create PreItems and ItemOrderItems
        for prod_round_id_str, qty in checkout_items.items():
            production_round_id = uuid.UUID(prod_round_id_str)
            cart_item = CartItem.query.filter_by(
                cart_id=customer.cart.id,
        ...
        if request.method == 'POST':
            ...
            # Transition PreItems to Items and remove corresponding CartItems
            for order_item in item_order.order_items:
                if order_item.item_type == "preitem":
                    pre_item = order_item.item

                    # Update the polymorphic identity to transition PreItem to Item
                    pre_item.__class__ = Item
                    pre_item.type = "item"

                    order_item.item_type = "item"


                    # Remove the corresponding CartItem
                    CartItem.query.filter_by(
                        cart_id=customer.cart.id,
                        production_round_id=pre_item.production_round_id
                    ).delete()
...
def cleanup_expired_orders():
    try:
        ...
        db.session.commit()
        flash("Expired orders cleaned up successfully.", "success")
