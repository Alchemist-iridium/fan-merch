...
def unpaid_item_order_wallet_payment(order_id):
    try:
        ...
        if request.method == 'POST':
            ...
            # Debugging: Log updates before commit
            logging.debug(f"Updated Wallet Balance: {customer.wallet_balance}")
            logging.debug(f"Updated Order Payment Status: {item_order.payment_status}, Payment Method: {item_order.payment_method}")
