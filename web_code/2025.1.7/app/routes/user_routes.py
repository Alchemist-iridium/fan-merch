...
def select_checkout_items():
    try:
        ...
        # 2) Get the list of selected cart items (by production_round_id)
        selected_items = request.form.getlist('selected_items')
        if not selected_items:
            flash("No items selected for checkout.")
            return redirect(url_for('user_interface.view_cart'))

        # 3) For each selected item, read the quantity from form
        #    Typically named like quantity_<production_round_id>
        session['checkout_items'] = {
            item_id_str: int(request.form.get(f'quantity_{item_id_str}', '1'))
...
def checkout():
    ...
    # Check session for customer role
    if 'user_id' not in session or session.get('role') != 'customer':
        logging.debug(
            "User is not logged in or is not a customer. "
            "user_id=%s, role=%s",
    ...
    try:
        customer_id = uuid.UUID(session['user_id'])
        customer = Customer.query.get_or_404(customer_id)
        logging.debug("Retrieved customer: %s", customer)
    except Exception as e:
        logging.exception("Error retrieving customer from session user_id.")
        flash("Invalid customer session. Please log in again.")
        return redirect(url_for('user_interface.login'))

    # 1) If GET, display the checkout page
    if request.method == 'GET':
        checkout_items = session.get('checkout_items', {})
        logging.debug("GET checkout_items in session: %s", checkout_items)
        ...
        # Gather CartItem objects from DB
        for prod_round_id_str, qty in checkout_items.items():
            logging.debug(
                "Processing item - production_round_id: %s, quantity: %s",
                prod_round_id_str, qty
            )
            try:
                prod_round_id = uuid.UUID(prod_round_id_str)
                cart_item = CartItem.query.filter_by(
                    cart_id=customer.cart.id,
                    production_round_id=prod_round_id
                ).first()
            except Exception as e:
                logging.exception("Error retrieving CartItem from DB.")
                continue

            if cart_item:
                cart_item_details.append(cart_item)
                total_price_for_item = (cart_item.unit_price * qty)
                total_amount += total_price_for_item
                logging.debug(
                    "Found cart_item=%s with unit_price=%s -> subtotal=%s",
                    cart_item, cart_item.unit_price, subtotal
                )

            else:
                logging.debug(
                    "No CartItem found for production_round_id=%s",
                    prod_round_id_str
                )

        logging.debug("Final cart_item_details: %s", cart_item_details)
        logging.debug("Final total_amount: %f", total_amount)
    ...
    # 2) If POST, attempt to finalize payment
    if request.method == 'POST':
        checkout_items = session.get('checkout_items', {})
        logging.debug("POST checkout_items in session: %s", checkout_items)

        if not checkout_items:
            logging.debug("No items selected for checkout on POST.")
            flash("No items selected for checkout.")
            return redirect(url_for('user_interface.view_cart'))

        payment_method = request.form.get('payment_method', 'wallet')
        logging.debug("Payment method from form: %s", payment_method)

        try:
            # Convert {str: int} to {UUID: int}
            cart_items_dict = {
                uuid.UUID(k): v for k, v in checkout_items.items()
            }
            logging.debug("Converted cart_items_dict: %s", cart_items_dict)

            # Create and pay the ItemOrder
            item_order = customer.create_item_order(
                cart_items=cart_items_dict,
                payment_method=payment_method
            )
            logging.debug("Created item_order: %s", item_order)

            # Remove purchased items from the cart
            for prod_round_id, qty in cart_items_dict.items():
                logging.debug("Removing item from cart: %s", prod_round_id)
                customer.cart.remove_item_from_cart(prod_round_id)

            db.session.commit()
            logging.debug("Order committed successfully. item_order_id=%s", item_order.id if item_order else None)

            # Clean up session
            session.pop('checkout_items', None)

            flash("Checkout successful!", "success")
            logging.debug("Checkout successful. Redirecting to dashboard.")
            return redirect(url_for('user_interface.customer_dashboard'))

        except Exception as e:
            logging.exception("Error occurred during payment finalization.")
            db.session.rollback()
...
def select_payment_method():
    logging.debug("Entered /select_payment_method route with method: %s", request.method)
    if 'user_id' not in session or session.get('role') != 'customer':
        logging.debug(
            "User is not logged in or is not a customer. "
            "user_id=%s, role=%s",
    ...
    logging.debug("selected_order_ids: %s, total_amount: %s", selected_order_ids, total_amount)

    if not selected_order_ids:
        logging.debug("No orders selected for payment.")
    ...
    if request.method == 'POST':
        ...
        if payment_method == 'wallet':
            return redirect(url_for('user_interface.pay_with_wallet'))
        elif payment_method == 'sandbox':
            return redirect(url_for('user_interface.pay_with_sandbox'))
        else:
            logging.debug("Invalid payment method selected.")
            flash("Invalid payment method selected.")
            return redirect(url_for('user_interface.select_payment_method'))
...
def pay_with_wallet():
    """
    Let the user confirm payment using their wallet balance.
    On POST, deduct the amount from wallet_balance and finalize the order.
    """
    logging.debug("Entered /pay_with_wallet route with method: %s", request.method)
    if 'user_id' not in session or session.get('role') != 'customer':
        logging.debug(
            "User is not logged in or is not a customer. "
            "user_id=%s, role=%s",
        ...
        flash("You need to be logged in as a Customer to use wallet payment.")
        return redirect(url_for('user_interface.login'))

    customer = Customer.query.get(uuid.UUID(session['user_id']))
    logging.debug("Retrieved customer for wallet payment: %s", customer)

    total_amount = session.get('total_amount', 0.0)
    order_id = session.get('pending_order_id', None)
    logging.debug("Wallet payment - total_amount=%s, order_id=%s", total_amount, order_id)

    if request.method == 'POST':
        logging.debug("User confirmed wallet payment.")
        if customer.wallet_balance < total_amount:
            logging.debug(
                "Insufficient wallet balance. Current balance=%s, required=%s",
                customer.wallet_balance, total_amount
            )
            flash("Insufficient wallet balance to complete payment.")
            return redirect(url_for('user_interface.order_payment_status', payment_status='failed'))
        else:
            # Deduct from wallet
            customer.wallet_balance -= total_amount
            logging.debug("Deducted %s from customer wallet. New balance=%s", total_amount, customer.wallet_balance)

            # If we have a pending ItemOrder, mark as paid
            if order_id:
                item_order = ItemOrder.query.get(order_id)
                if item_order:
                    item_order.payment_status = 'paid'
                    item_order.payment_method = 'wallet'
                    item_order.payment_timestamp = datetime.now()
                    logging.debug("ItemOrder %s marked as paid (wallet).", order_id)

            db.session.commit()
            logging.debug("Wallet payment committed successfully.")

            flash(f"Payment of ${total_amount} completed using your wallet.")
            return redirect(url_for(
                'user_interface.order_payment_status', 
    ...
    # If GET, just show a confirmation page
    logging.debug("Rendering pay_with_wallet confirmation page.")
    return render_template(
        'user/pay_with_wallet.html',
        total_amount=total_amount,
...
# need update on Wallet, as it is an attribute in customer right now...
# probably the wallet would be removed from the final system, as holding money in system is too dangerous
